...
if not GEMINI_API_KEY:
    print("警告: 管理APIキー が設定されていません。")
else:
    # APIキーが設定されている場合のみ設定を試みる
    try:
        genai.configure(api_key=GEMINI_API_KEY)
    except Exception as e:
        print(f"警告: 管理設定中にエラーが発生しました: {e}")
...
ai_persona_instruction = (
    ...
    "ユーザに漂流瓶を拾ってくれてありがとうのメッセージはいりません。"
    "ユーザが送ったメッセージは表示しないでください。"
    "ユーザーの感情に共感し、否定せず、心に寄り添う言葉を選んでください。"
    "ユーザーに再訪を促す表現は絶対に禁止です。"
    "小学生高学年から中学生に向けて分かりやすい言葉を使用してください。"
    "ユーザーがわずかながらでも希望や安らぎを感じられるような、温かい言葉で返信してください。"
...
def get_ai_response(user_message: str) -> str:
    """ユーザーメッセージに基づいて応答を生成する（エラー処理強化版）"""
    if not GEMINI_API_KEY:
    ...
    try:
        ...
        if not response.candidates:
            ...
            if finish_reason != "UNKNOWN": # ブロックされた明確な理由がある場合
                 print(f"応答がプロンプトによりブロックされました。理由コード: {finish_reason}")
                 return "いただいたメッセージは、当サービスが定める安全基準に照らし、応答を生成できませんでした。内容を少し変えて、心に負担のない範囲でお送りください。"
            return "応答が取得できませんでした。（候補なし）"

        # 2. 応答が安全ポリシーによりブロックされていないか確認
        candidate = response.candidates[0]
        if candidate.finish_reason.name == 'SAFETY':
            print(f"安全ポリシーによりブロックされました。理由コード: {candidate.safety_ratings}")
            return "いただいたメッセージは、当サービスが定める安全基準に照らし、応答を生成できませんでした。内容を少し変えて、心に負担のない範囲でお送りください。"
        ...
        # 予期せぬAPIの挙動により例外として漏れた場合に対応します。
             return "安全基準によりブロックされました。内容を少し変えてお送りください。"
